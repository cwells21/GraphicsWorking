...
public class PCircle extends JFrame implements GLEventListener
{
	...
	public void generateVertexes() 
	{
		...
		int steps = (int)Math.pow(4, index)*3;
		double delta = (double)((2*Math.PI)/steps);
		int currentPos = 0;
		while ( currentPos < steps)
		{
			VertArray[currentPos][0] = (this.radius * (float)Math.cos(currentPos * delta));
			VertArray[currentPos][1] = (this.radius * (float)Math.sin(currentPos * delta));
			VertArray[currentPos][2] = 0.0f;
